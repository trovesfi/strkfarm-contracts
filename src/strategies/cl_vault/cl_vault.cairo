...
mod ConcLiquidityVault {
  ...
  impl ExternalImpl of IClVault<ContractState> {
    ...
    // @param new_bounds: new bounds to set
    // @param swap_params: swap params to adjust current balances as per new bounds
    fn rebalance(
      ref self: ContractState, 
      new_bounds: Bounds,
      ...
      println!("bol0 before swap {:?}", bal0);
      println!("bol1 before swap {:?}", bal1);
    ...
    fn handle_unused(
      ...
      assert(
        ...
        swap_params.token_from_address == pool_key.token1, 'invalid swap params [1]');
      assert(
        swap_params.token_to_address == pool_key.token0 ||
        swap_params.token_to_address == pool_key.token1, 'invalid swap params [2]');
